...
// De sessie zit in een cookie; die moeten we zelf meenemen want fetch doet dat niet.
const cookie = (login.headers.getSetCookie?.() || [login.headers.get("set-cookie")])
  .filter(Boolean).map((c) => c.split(";")[0]).join("; ");
if (!cookie) { console.error("Geen sessiecookie ontvangen na inloggen."); process.exit(1); }
