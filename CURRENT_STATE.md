...
Op verzoek, 2026-09-12: testchatbot voor prospect Benn Fietsen (bennfietsen.nl,
WordPress/WooCommerce) lokaal gebouwd op de bestaande chatbotserver. Kennisbank
wordt gegenereerd uit de open REST API's van de site (`product/chatbot/tools/
build-kb-wordpress.mjs`), twintig testvragen, drie modellen vergeleken:
gemini-2.5-flash wint, flash-lite afgevallen. Plan en testverslag in
`clients/bennfietsen/docs/`. Eindrondes gehaald (twee keer nul verzonnen feiten), eigen Benn-testsite als
premium pagina (GSAP, Lenis), sinds 2026-09-12 live op bennfietsen.belvanger.nl
(noindex, container bennfietsen, poort 8093, deploy in clients/bennfietsen/deploy).
Aanbod: 625 setup plus 99 per maand als launchklant, afspraak als PDF in
clients/bennfietsen/docs. Niets gecommit.
...
- SABL-migratie naar Robinhood Chain (12 sep): founder meldt dat het Sable-team een volledige migratie plus bridge naar chain 4663 aankondigt; niet gevonden op buildsable.com of in de whitepaper (18:30Z), wel drie namaak-SABL's op 4663 in de counterfeit-ledger (12 sep een SABLE-pool van ~$9,7K). Advies geschreven in crypto/research/2026-09-12-sabl-migration-distribution.md: geen snapshot, geen deadline, geen two-way bridge, maar eenrichtings burn-and-mint 1:1 zonder sluitdatum, harde cap gelijk aan de Solana-supply (958,36M op 12 sep, authorities revoked), elke token op 4663 gedekt door een burn op Solana, publieke verifier plus dagelijkse mint-limiet, gas betaald door de relayer, claim via Phantom (steunt 4663 sinds 23 jul), officieel adres eerst op de whitepaper-pagina, live bewijspagina burned-vs-minted (Observatorium kan die hosten). Aankondiging bevestigd: @sablenetwork op X, 12 sep 17:43Z (status 2098829794276221119), details volgen. Op verzoek van de founder als steunend stuk voor het team herschreven (blueprint, geen waakhond-toon) en klaargezet voor de Reading room: crypto/content/2026-09-12-sabl-migration/article.md + article.pdf (render-pdf.mjs met eigen kop en datum), notes/2026-09-12-sabl-migration.md + .pdf, build-notes en build gedraaid, page-log-regel toegevoegd, tests/shot-note.mjs (nieuw: titel, secties, tien stappen, PDF-link, geen overflow, geen em-dash, bovenaan de lijst, PDF geserveerd) en shot-reading groen lokaal. Niet gemeten: holders en top-holder-aandeel (publieke RPC's weigeren de call). Op "push" gedeployed naar de VPS (12 sep 18:50Z, board-data op volume, brief van Lisa bewaard: count 1, geen fout) en live gecontroleerd met shot-note.mjs (PDF 200, 231 KB); Sable-werk gecommit en gepusht als aa2c094.
...
- Hoezen op de site en Lisa (12 sep, avond): de luisterkamer toont het albumhoes met titel, artiest en een knop "Play the EP" boven de lijst; elke kaart draagt zijn hoes als thumbnail (grid: hoes links, tekst rechts, speler en knoppen over de volle breedte) en na een nummer start het volgende. build-tracks.mjs kopieert covers/web/album.jpg mee; index.json draagt per nummer de cover; de statische testserver kent nu jpeg. Eerste speler tekende in headless Chrome zijn smalle layout tot een scroll (boxmodel klopte, 285 px): controls-attribuut wordt na het renderen opnieuw gezet. Lisa: geen ElevenLabs-sleutel lokaal en AG is eigenaar van de agent, dus een bericht voor AG met de volledige nieuwe systeemprompt (alle topics sinds 7 sep: reliability record, burn watch, chart, kaarten in The field, referee en Java-tab, leeskamer, luisterkamer, brievenbus, geluiden) plus de volledige topic-lijst voor de navigate-tool: `crypto/content/2026-09-12-lisa-listening-room-update.md`; LISA-GUIDE.md en LISA-PROMPT.txt bijgewerkt. Commits lokaal: 56b7cb7 (luisterkamer, ident, hoezen, plus brievenbus- en leeskamerbestanden van 10 en 11 sep), 9974692 en 5c60cb6 (Lisa); push door de founder.
