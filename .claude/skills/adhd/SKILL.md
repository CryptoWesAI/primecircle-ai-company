...
original prose is preserved in the upstream repo's `SOURCE-SPEC.md` for
reference. The implementation choices made here (parallel isolated Agent
calls, mechanical generator/critic split, frame-based branching) follow
from that spec.

<!-- Provenance: third-party skill, MIT-licensed, vendored 2026-07-22 from
github.com/UditAkhourii/adhd. Not authored by us. Instruction-only (no
bundled code executes); the optional npm package `adhd-agent` is separate
and NOT installed. -->
