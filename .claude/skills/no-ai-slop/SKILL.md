...
<!-- Provenance: third-party skill, MIT-licensed, vendored 2026-07-28 from
github.com/petergyang/no-ai-slop (commit e81170d). Not authored by us.
Instruction-only; eval.md hoort erbij en wordt door stap 4 van de workflow gebruikt. -->

## Gotchas (toevoeging van ons, niet van de auteur)

**De woordenlijsten zijn Engels en dit project schrijft Nederlands.** Dat is dezelfde
valkuil als bij `scroll-film-studio/scripts/copy-gate.js`, die op 27 juli niets ving op
onze pagina's omdat alle regels Engelstalig waren. Bij het toepassen op Nederlandse tekst
moet je de Nederlandse tegenhangers erbij denken: "naadloos", "krachtig", "baanbrekend",
"in een notendop", "het is belangrijk om op te merken", "aan het eind van de dag",
"kortom", "tot slot", "uiteindelijk". En de binaire tegenstelling heet hier "het is niet
X, het is Y" of "de vraag is niet X maar Y".

**Em-dashes zijn hier een harde regel, geen voorkeur.** De founder wil ze niet zien. Waar
de skill zegt "1-2 zijn prima in langere teksten", geldt hier: nul.

**Detect voordat je edit op bestaand materiaal.** Bij een audit over tientallen bestanden
is automatisch herschrijven gevaarlijk: de regels raken ook citaten, commandovoorbeelden
en logregels waar de tekst juist letterlijk moet blijven. Scan eerst, beoordeel per treffer,
corrigeer daarna met de hand.

**Vaktermen zijn geen slop, ook al staan ze op de lijst.** Bij de audit van 28 juli sloeg de
lijst tien keer aan op "leverage" en "harness". Alle tien onterecht: *founder leverage* is een
principe uit `PROJECT_KERNEL.md` en *verification harness* is een testopstelling. De lijst
mikt op het werkwoord ("leverage our platform"), niet op het zelfstandig naamwoord in een
vastgelegde term. Kijk naar de betekenis in de zin voordat je schrapt.

**Skills en documenten van derden niet herschrijven.** `adhd`, `scroll-world` en
`scroll-film-studio` zijn ingekochte teksten met een herkomstregel. Daar zaten 353 em-dashes
in en die zijn bewust blijven staan: het is andermans tekst, en herschrijven maakt een
volgende update uit de upstream-repo onmogelijk te vergelijken.

**Twee gereedschappen in `scripts/`**, allebei geschreven tijdens die audit:

- `slop-scan.mjs` meldt per regel welk patroon aanslaat, in NL en EN. Melden, niet wijzigen.
- `dash-fix.mjs` vervangt em-dashes, maar ALLEEN in labelvormen (kop, vetgedrukte term,
  code-term, opsomming). Wat midden in een zin staat blijft staan en wordt gemeld, want daar
  hangt het van de zin af of het een komma, een dubbele punt of een punt moet worden.
  Twee dingen die hij tijdens het bouwen fout deed en nu afvangt: een regel die al een dubbele
  punt had kreeg er een tweede, en een kop met twee dashes kreeg twee dubbele punten.
