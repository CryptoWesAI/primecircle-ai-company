...
const RULES = [
  ...
  // --- NEDERLANDS (lokale toevoeging, PrimeCircle 2026-07-27) ---
  // De originele regels zijn volledig Engelstalig, dus op een Nederlandse pagina ving
  // de poort NIETS. Bewezen: "Scroll verder en kijk wat er gebeurt, dan zie je hoe wij
  // verven" kwam er schoon door, terwijl dat precies de fout is die deze poort bestaat
  // om te vinden. Onze klantsites zijn allemaal Nederlands, dus zonder dit is de poort
  // hier decoratie.
  [/\bscroll(t|en)?\s+(verder|door|maar|naar\s+beneden|omlaag)\b/i, "zegt de bezoeker dat hij moet scrollen"],
  [/\bblijf\s+scrollen\b/i, "zegt de bezoeker dat hij moet scrollen"],
  [/\b(terwijl|als|wanneer)\s+(je|u)\s+(naar\s+beneden\s+)?scrollt\b/i, "vertelt de scroll in plaats van hem te laten werken"],
  [/\bkijk\s+(mee\s+)?wat\s+er\s+gebeurt\b/i, "kondigt het truukje aan in plaats van het te doen"],
  [/\bkijk\s+mee\s+terwijl\b/i, "kondigt het truukje aan in plaats van het te doen"],
  [/\bdeze\s+(pagina|site|website)\s+(is|was|laat|toont|legt|vertelt|neemt)\b/i, "de pagina heeft het over zichzelf"],
  [/\b(één|een)\s+doorlopende\s+(beweging|opname|scroll|reis|shot)\b/i, "beschrijft het mechaniek als tekst"],
  [/\bhoe\s+(je|u)\s+deze\s+(pagina|site)\s+leest\b/i, "geeft de bezoeker een leesinstructie"],
  [/\b(de\s+)?camera\s+(beweegt|zakt|vliegt|draait|gaat|komt)\b/i, "beschrijft de camera in bezoekerstekst"],
...
//
// LOKALE FIX (PrimeCircle, 2026-07-27) — de originele versie streepte alleen script,
// style en commentaar weg en liet dus de HÉLE MARKUP staan: klassenamen, attributen,
// data-attributen. Daardoor gaf de poort vals alarm op volkomen normale pagina's,
// bewezen met minimale testgevallen:
//   class="reveal"                 → "shot-list vocabulary as copy"
//   placeholder="Voor- en achternaam" → "placeholder marker left in copy"
//   data-pan="1"                   → "names a camera move in visitor-facing copy"
// Gevolg: elke pagina met een formulier faalde altijd, en `.reveal` is nog wel de
// klassenaam die deze skill in Lane A zélf voorschrijft. Een poort die niet groen kán
// worden, wordt uitgezet, en dan meet je niets meer.
//
// Nu wordt eerst alle markup verwijderd. Alt-, aria-label- en title-teksten zijn wél
// bezoekersteksten (schermlezers lezen ze voor), dus die halen we er als losse tekst
// weer bij in plaats van ze mee weg te gooien.
const visibleText = (html) => {
  const zonderCode = html
    .replace(/<script[\s\S]*?<\/script>/gi, " ")
    .replace(/<style[\s\S]*?<\/style>/gi, " ")
    .replace(/<!--[\s\S]*?-->/g, " ");
  const voorgelezen = [...zonderCode.matchAll(/\s(?:alt|aria-label|title)="([^"]*)"/gi)]
    .map((m) => m[1])
    .join("\n");
  return zonderCode.replace(/<[^>]+>/g, " ") + "\n" + voorgelezen;
};
